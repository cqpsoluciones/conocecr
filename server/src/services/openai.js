...
const generarRespuesta = async (mensaje, negocios, historial, userLat, userLng, intencion, userNombre, historialUsuario, perfilUsuario) => {
  ...
  const systemPrompt = [
    ...
    historialUsuario
      ...
      : '',
    perfilUsuario
      ? '\n# LO QUE SABÉS DE ESTA PERSONA (aprendido con el tiempo)\n\n' + perfilUsuario + '\n\nEsto es contexto que enriquece la conversación, NUNCA un atajo para dejar de preguntar. Seguí preguntando presupuesto, compañía y momento como siempre — la diferencia es que ahora podés hacerlo con más calidez y familiaridad, como quien ya conoce a la persona ("¿y esta vez para cuántos van?", con el tono de quien ya sabe que suele salir con amigos). Si la persona responde algo distinto a su costumbre habitual, tomá eso como la verdad de HOY, no lo que sabías de antes. Nunca le digas a la persona "según tu perfil" ni le recites esta información.'
      : '',
    '',
    negocios.length > 0
    ...
    '',
    '1. COMPRENDÉ ANTES DE RECOMENDAR (regla estructural, no opcional). Para CUALQUIER búsqueda de comida, salidas o experiencias, SIEMPRE preguntá lo necesario antes de recomendar: (a) presupuesto para esta ocasión, (b) con quién va o para qué ocasión, (c) si es para ahora o para otro momento — sin importar lo que sepás de su perfil. Nunca asumas ni recomiendes directamente saltándote esto: cada salida puede ser distinta a su costumbre. ÚNICA EXCEPCIÓN: pedidos urgentes y específicos ("qué está abierto ya", "algo rápido ahora") → respondé de inmediato.',
